...
export interface Session {
  /**
   * Optional. State set by Interactive Canvas web app. Only set for request, not for
   * response. For example, if this is a recipe app, the state can be a value of struct : {
   * "current_page" : 5, "last_page" : 3, }
   */
  canvasState?: any;
  /**
...
export interface TypeOverride {
  ...
  synonym?: SynonymType;
  /**
   * Deprecated name for TypeOverrideMode due to a problem with enum type name generation
   */
...
export interface Prompt {
   ...
   * "false" will clear all previously defined messages (first and last simple, content,
   * suggestions link and canvas) and add messages defined in this prompt. "true" will add
   * messages defined in this prompt to messages defined in previous responses. Setting this
   * field to "true" will also enable appending to some fields inside Simple prompts, the
   * Suggestion prompt and the Canvas prompt (part of the Content prompt). The Content and
   * Link messages will always be overwritten if defined in the prompt. Default value is
   * "false".
   */
  append?: boolean;
  /**
